    ...
    try: # STLport 5.0 and later, _STL_DEBUG defined
        ...
        try: # STLport 5.0 and later, _STL_DEBUG undefined
            ...
            try:
                ...
                return gdb.lookup_type ('_SASSTL::' + typename)
...
class MapPrinter:
    ...
    def children (self):
        try:
            ...
            pair_type = self.val.type.template_argument(3).template_argument(0)
        except:
            # So use this as workaround:
            pair_type = self.val['_M_t'].type.template_argument(1)
        node_type  \
...
def add_entry (regex, printer, typename):
    prefix = "^(stlpd?_std|_STL|_SASSTL)::"
    suffix = "<.*>$"
    if typename != None:
...
add_entry ("pair",         PairPrinter,  "pair")
